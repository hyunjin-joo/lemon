...
<link rel="shortcut icon"
	...
	href="<%=request.getContextPath() %>/css/head.css">
		<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath() %>/css/list_article.css">
	<link rel="stylesheet" type="text/css"
...
<c:forEach var="article" items="${articlePage.content}">
	<tr>
		<td class="no">${article.number}</td>
		<td class="wrap">
		<a href="read.do?no=${article.number}&pageNo=${articlePage.currentPage}">
		<c:out value="${article.title}"/>
		</a>
		</td>
		<td class="no">${article.writer.name}</td>
		<td class="no"><fmt:formatDate pattern="yyyy-MM-dd" value="${article.regDate}" /></td>
		<td class="no">${article.readCount}</td>
	</tr>
</c:forEach>
</table>  
</div>
